...
export default class Details extends DJILogRecordBase {
  constructor(buffer: ByteBuffer, index: number, key?: number[]) {
    super(buffer, index, key);
  }

  getSubStreet() {
    return this.buffer.toString("utf8").slice(this.index, this.index+20);
  }
  
  getStreet() {
    return this.buffer.toString("utf8").slice(this.index+20, this.index+40);
  }
  
  getCity() {
    return this.buffer.toString("utf8").slice(this.index+40, this.index+60);
  }
  
  getArea() {
    return this.buffer.toString("utf8").slice(this.index+60, this.index+80);
  }
  ...
  getAircraftSn() {
    return this.buffer.toString("utf8").slice(this.index+267, this.index+277);
  }
  
  getAircraftName() {
    return this.buffer.toString("utf8").slice(this.index+278, this.index+302);
  }
  ...
  getCameraSn() {
    return this.buffer.toString("utf8").slice(this.index+318, this.index+328);
  }
  
  getRcSn() {
    return this.buffer.toString("utf8").slice(this.index+328, this.index+338);
  }
  
  getBatterySn() {
    return this.buffer.toString("utf8").slice(this.index+338, this.index+348);
  }
